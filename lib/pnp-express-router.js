...
RouteLoader.prototype._buildPluginArgs = function (routeType, plugin, pluginArgs) {
    ...
    httpHandlers = methods.slice();
    httpHandlers.push('del', 'all');

    appFns = httpHandlers.slice();
    appFns.push('param', 'use', 'namespace');
    ...
    appFns.forEach(function (fn) {
        routerApp[fn] = function () {
            ...
            if (httpHandlers.indexOf(fn) > -1) {

                if (this._ns.length) {
                    var ns = this._ns.slice();
                    ns.push(path);

                    path = ns.join('/').replace(/(\/){2,}/g, '/').replace(/\/$/, '') || '/';
                }
